...
void t_PrintHeader(FILE * f) {
		fprintf(f, "function name|input size|real time|user time|system time\n");
}
...
void t_Print(timing * t, FILE * f, const char * functionName, int inputSize) {
	...
	float system_time = (t->rend.ru_stime.tv_sec - t->rstart.ru_stime.tv_sec) + 1e-6 * (t->rend.ru_stime.tv_usec - t->rstart.ru_stime.tv_usec);
	fprintf(f, "%s|%d|%lld|%0.6f|%0.6f\n", functionName, inputSize, elapsed, user_time, system_time);
}
